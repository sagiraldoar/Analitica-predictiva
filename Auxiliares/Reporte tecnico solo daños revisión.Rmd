...
Sin embargo al persistir el sobre entrenamiento  y teniendo en cuenta que la mayoría de estos son mayores al 5% (variables no son significativas para el modleo ), se debe realizar una selección de variables para tener un modelo más sencillo pero con gran capacidad predictiva, por lo anterior se parte de este modelo base sin año de referencia y sin semana, y se realiza de nuevo una selección de variables utilizando el stepwise
...
Este modelo de regresión lineal con selección de variables mejora considerablemente el AIC pues pasa de 10918.18 a 10887.6 lo que es muy positivo porque este criterio tiene en cuenta tanto la capacidad de predicción como el número de variables,aunque se genera un sobrentrenamiento del 16%, el cual aunque se acerca al valor objetivo del 15% aún se deben realizar ajustes, para solucionar el inconveniente
...
```{r}
df_modelo_importancia_rf=df_modelo[,c('FECHA','PERIODO','FESTIVO','LABORAL','SEMANA_SANTA','MES_01','DIA_NOMBRE_DOMINGO','DIA_NOMBRE_SÁBADO','DIA_NOMBRE_VIERNES','NRO_ACCIDENTES' )]
```
