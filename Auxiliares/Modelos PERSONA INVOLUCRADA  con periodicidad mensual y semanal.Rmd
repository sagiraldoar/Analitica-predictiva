...
###MODELACIÓN CON REGRESIÓN LINEAL LOGARITMICA UTILIZANDO SELECCIÓN DE VARIABLES CON EL METODO STEPWISE PARA LA PERIODICIDAD MENSUAL
...
###MODELACIÓN CON REGRESIÓN LINEAL LOGARITMICA UTILIZANDO SELECCIÓN DE VARIABLES CON EL METODO STEPWISE PARA LA PERIODICIDAD SEMANAL


```{r}
modelo_lm_log <- lm(log(NRO_ACCIDENTES)~. - PERIODO - SEMANA_01,data=trains)
summary(modelo_lm_log)
cat("AIC_",AIC(modelo_lm_log))
pred=predict(modelo_lm_log,trains)
cat(" MSE_t:",mean((exp(pred)-trains$NRO_ACCIDENTES)^2))
pred=predict(modelo_lm_log,validations)
cat(" MSE_v:",mean((exp(pred)-validations$NRO_ACCIDENTES)^2))
```
...
```{r}
modelo_lm_log <- lm(formula(modelo_lm_log_step),data=trains)
summary(modelo_lm_log)
cat("AIC:",AIC(modelo_lm_log))
pred=predict(modelo_lm_log,trains)
cat(" MSE_t",mean((exp(pred)-trains$NRO_ACCIDENTES)^2))
